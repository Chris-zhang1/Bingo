...
class BLOCK {
  public:
    ...
    uint64_t address, full_addr, tag, data, cpu, instr_id;
...
class PACKET {
  public:
    ...
    uint8_t is_producer, rob_index_depend_on_me[ROB_SIZE], lq_index_depend_on_me[ROB_SIZE],
        sq_index_depend_on_me[ROB_SIZE], instr_merged, load_merged, store_merged, returned, asid[2], type;

    uint32_t cpu, data_index, lq_index, sq_index;

    uint64_t address, full_addr, instruction_pa, data_pa, data, instr_id, ip, event_cycle;
...
class PACKET_QUEUE {
  public:
    ...
    uint32_t cpu, head, tail, occupancy, num_returned, next_fill_index, next_schedule_index, next_process_index;

    uint64_t next_fill_cycle, next_schedule_cycle, next_process_cycle, ACCESS, FORWARD, MERGED, TO_CACHE,
        ROW_BUFFER_HIT, ROW_BUFFER_MISS, FULL;
...
class CORE_BUFFER {
  public:
    const string NAME;
    const uint32_t SIZE;
    uint32_t cpu, head, tail, occupancy, last_read, last_fetch, last_scheduled, inorder_fetch[2], next_fetch[2],
        next_schedule;
    uint64_t event_cycle, fetch_event_cycle, schedule_event_cycle, execute_event_cycle, lsq_event_cycle,
        retire_event_cycle;
...
class LSQ_ENTRY {
  public:
    ...
    uint8_t translated, fetched, asid[2], forwarding_depend_on_me[ROB_SIZE];
