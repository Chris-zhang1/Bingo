...
void finish_warmup()
{
    ...
    uncore.STLB.LATENCY = STLB_LATENCY;

    uncore.LLC.llc_prefetcher_inform_warmup_complete();	
}
...
uint64_t va_to_pa(uint32_t cpu, uint64_t instr_id, uint64_t va, uint64_t unique_vpage)
{
    ...
    if (pr == page_table.end()) { // no VA => PA translation found 
        ...
        if (allocated_pages >= DRAM_PAGES) { // not enough memory
            ...
            cout << "[SWAP] update page table NRU_vpage: " << hex << pr->first << " new_vpage: " << vpage << " ppage: " << pr->second << dec << "\tcpu = " << cpu << endl; });
            ...
            cout << ppage_check->second << " ppage: " << ppage_check->first << dec << "\tcpu = " << cpu << endl; });
            ...
            uncore.STLB.invalidate_entry(NRU_vpage);
            ...
            while (1) { // try to find an empty physical page number
                ...
                if (ppage_check != inverse_table.end()) { // random_ppage is not available
                    DP ( if (warmup_complete[cpu]) {
                    cout << "vpage: " << hex << ppage_check->first << " is already mapped to ppage: " << random_ppage << dec << "\tcpu = " << cpu << endl; }); 
            ...
            if (fragmented) {
                ...
                DP ( if (warmup_complete[cpu]) {
                cout << "Recalculate num_adjacent_page: " << num_adjacent_page << "\tcpu = " << cpu << endl; });
            }
        }
    ...
    cout << " => ppage: " << (pa >> LOG2_PAGE_SIZE) << " vadress: " << unique_va << " paddress: " << pa << dec << "\tcpu = " << cpu << endl; });
...
int main(int argc, char** argv)
{
    ...
    for (int i=0; i<NUM_CPUS; i++) {
        ...
        ooo_cpu[i].ITLB.lower_level = &uncore.STLB; 
        ...
        ooo_cpu[i].DTLB.lower_level = &uncore.STLB;

        //uncore.STLB.cpu = i;
        uncore.STLB.cache_type = IS_STLB;
        uncore.STLB.fill_level = FILL_LLC;
        uncore.STLB.upper_level_icache[i] = &ooo_cpu[i].ITLB;
        uncore.STLB.upper_level_dcache[i] = &ooo_cpu[i].DTLB;
        ...
        ooo_cpu[i].L1I.lower_level = &uncore.LLC; 
        ...
        ooo_cpu[i].L1D.lower_level = &uncore.LLC; 
        ...
        uncore.LLC.upper_level_icache[i] = &ooo_cpu[i].L1I;
        uncore.LLC.upper_level_dcache[i] = &ooo_cpu[i].L1D;
        ...
        uncore.LLC.llc_prefetcher_initialize(i);
    }
    ...
    while (run_simulation) {
		...
		uncore.LLC.llc_prefetcher_inform_roi_complete(i);  
    ...
    for (int i = 0; i < NUM_CPUS; i += 1) {
        ...
        uncore.LLC.llc_prefetcher_final_stats(i);
    }

    for (int i = 0; i < NUM_CPUS; i += 1) {
        //ooo_cpu[i].L1D.l1d_prefetcher_roi_stats();
        //ooo_cpu[i].L2C.l2c_prefetcher_roi_stats();
        uncore.LLC.llc_prefetcher_roi_stats(i);
    }
