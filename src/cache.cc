...
void CACHE::handle_fill()
{
    ...
    if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu]) {
        ...
        if (block[set][way].dirty) {
            ...
            if (lower_level) {
                if (lower_level->get_occupancy(2, block[set][way].address) == lower_level->get_size(2, block[set][way].address)) {
                    ...
                    cout << " victim_addr: " << block[set][way].tag << dec << "\tfill_cpu = " << fill_cpu << endl; });
                }
                else {
        ...
        if (do_fill) {
            ...
            else if (cache_type == IS_LLC)//Bingo
                llc_prefetcher_cache_fill(fill_cpu, MSHR.entry[mshr_index].full_addr, set, way, (MSHR.entry[mshr_index].type == PREFETCH) ? 1 : 0, block[set][way].full_addr);//Bingo
...
void CACHE::handle_writeback()
{
    ...
    if ((WQ.entry[WQ.head].event_cycle <= current_core_cycle[writeback_cpu]) && (WQ.occupancy > 0)) {
        ...
        if (way >= 0) { // writeback hit (or RFO hit for L1D)
            ...
            cout << " cycle: " << WQ.entry[index].event_cycle << "\twriteback_cpu = " << writeback_cpu << endl; });
            ...
            if (cache_type == IS_L1D) { // RFO miss
                ...
                if (block[set][way].dirty) {
                    ...
                    if (lower_level) { 
                        if (lower_level->get_occupancy(2, block[set][way].address) == lower_level->get_size(2, block[set][way].address)) {
                            ...
                            cout << " victim_addr: " << block[set][way].tag << dec << "\twriteback_cpu = " << writeback_cpu  << endl; });
                        }
                        else { 
                            PACKET writeback_packet;
                ...
                if (do_fill) {
                    ...
                    if (cache_type == IS_L1D)
                        ...
                        llc_prefetcher_cache_fill(writeback_cpu, WQ.entry[index].full_addr, set, way, 0, block[set][way].full_addr);//Bingo
            ...
            if (way >= 0) { // read hit
                ...
                if (RQ.entry[index].type == LOAD) {
                    if (cache_type == IS_L1D) 
                        ...
                        llc_prefetcher_operate(read_cpu, block[set][way].full_addr, RQ.entry[index].ip, 1, RQ.entry[index].type);//Bingo
                }
                ...
                cout << " cycle: " << RQ.entry[index].event_cycle << "\tread_cpu = " << read_cpu  << endl; });
                    ...
                    if ((mshr_index == -1) && (MSHR.occupancy == MSHR_SIZE)) { // not enough MSHR resource
                        ...
                        if (RQ.entry[index].type == RFO) {
                            ...
                            if (RQ.entry[index].instruction) {
                                ...
                                cout << " merged rob_index: " << rob_index << " instr_id: " << RQ.entry[index].instr_id << "\tMSHR.entry[mshr_index].cpu = " << MSHR.entry[mshr_index].cpu  << endl; });
                                ...
                                if (RQ.entry[index].instr_merged) {
                                    for (uint32_t i=0; i<ROB_SIZE; i++) {
                                        if (RQ.entry[index].rob_index_depend_on_me[i]) {
                                            ...
                                            DP (if (warmup_complete[MSHR.entry[mshr_index].cpu]) {
                                            cout << "[INSTR_MERGED] " << __func__ << " cpu: " << MSHR.entry[mshr_index].cpu << " instr_id: " << MSHR.entry[mshr_index].instr_id;
                                            cout << " merged rob_index: " << i << " instr_id: N/A" << "\tMSHR.entry[mshr_index].cpu = " << MSHR.entry[mshr_index].cpu  << endl; });
                                        }
                                    }
                                }
                            }
                            else 
                            {
                                ...
                                cout << " merged rob_index: " << RQ.entry[index].rob_index << " instr_id: " << RQ.entry[index].instr_id << " lq_index: " << RQ.entry[index].lq_index << "\tread_cpu = " << read_cpu  << endl; });
                ...
                if (miss_handled) {
                    ...
                    if (RQ.entry[index].type == LOAD) {
                        if (cache_type == IS_L1D) 
                            ...
                            llc_prefetcher_operate(read_cpu, RQ.entry[index].full_addr, RQ.entry[index].ip, 0, RQ.entry[index].type);//Bingo
                    }
    ...
    for (uint32_t i=0; i<MAX_READ; i++) {
        ...
        if ((PQ.entry[PQ.head].event_cycle <= current_core_cycle[prefetch_cpu]) && (PQ.occupancy > 0)) {
            ...
            if (way >= 0) { // prefetch hit
                ...
                cout << " cycle: " << PQ.entry[index].event_cycle << "\tprefetch_cpu = " << prefetch_cpu << endl; });
                ...
                if ((mshr_index == -1) && (MSHR.occupancy < MSHR_SIZE)) { // this is a new miss
                    ...
                    cout << " occupancy: " << lower_level->get_occupancy(3, PQ.entry[index].address) << " SIZE: " << lower_level->get_size(3, PQ.entry[index].address) << "\tPQ.entry[index].cpu = " << PQ.entry[index].cpu << endl; });
                    ...
                    if (lower_level) {
                        if (cache_type == IS_LLC) {
                            ...
                            if (lower_level->get_occupancy(3, PQ.entry[index].address) == lower_level->get_size(3, PQ.entry[index].address))
                                ...
                                lower_level->add_pq(&PQ.entry[index]); // add it to the DRAM RQ. [Bingo: The comment is wrong!!!]
                            }
                        }
                    }
                }
                    ...
                    if ((mshr_index == -1) && (MSHR.occupancy == MSHR_SIZE)) { // not enough MSHR resource
                        ...
                        cout << " cycle: " << MSHR.entry[mshr_index].event_cycle << "\tprefetch_cpu = " << prefetch_cpu << endl; });
                    }
                    else { // WE SHOULD NOT REACH HERE
                        cerr << "[" << NAME << "] MSHR errors" << endl;
                        assert(0);
...
int CACHE::add_rq(PACKET *packet)
{
    ...
    if (wq_index != -1) {
        ...
        if ((cache_type == IS_L1D) && (packet->type != PREFETCH)) {
            ...
            cout << " index: " << MAX_READ << " rob_signal: " << packet->rob_signal << "\tpacket->cpu = " << packet->cpu << endl; });
        }
    ...
    if (index != -1) {
        ...
        if (packet->instruction) {
            ...
            cout << " merged rob_index: " << rob_index << " instr_id: " << packet->instr_id << "\tpacket->cpu = " << packet->cpu << endl; });
        }
        else 
        {
            // mark merged consumer
            if (packet->type == RFO) {
                ...
                cout << " merged rob_index: " << packet->rob_index << " instr_id: " << packet->instr_id << " lq_index: " << packet->lq_index << "\tpacket->cpu = " << packet->cpu  << endl; 
		cout << "[Bingo] With whom I merged?" << "\tindex = " << index << "\tRq.entry[index].instr_id = " << RQ.entry[index].instr_id << "\tRQ.entry[index].address = " << hex << RQ.entry[index].address << "\tRQ.entry[index].full_addr = " << RQ.entry[index].full_addr << "\tRQ.entry[index].cpu = " << RQ.entry[index].cpu << endl;
		});
            }
        }
    ...
    cout << " event: " << RQ.entry[index].event_cycle << " current: " << current_core_cycle[RQ.entry[index].cpu] << "\tRQ.entry[index].cpu = " << RQ.entry[index].cpu << endl; });
...
int CACHE::add_wq(PACKET *packet)
{
    ...
    cout << " event: " << WQ.entry[index].event_cycle << " current: " << current_core_cycle[WQ.entry[index].cpu] << "\tWQ.entry[index].cpu = " << WQ.entry[index].cpu  << endl; });
...
int CACHE::add_pq(PACKET *packet)
{
    ...
    if (PQ.occupancy == PQ_SIZE) {
        ...
        DP ( if (warmup_complete[packet->cpu]) {
        cout << "[" << NAME << "] cannot process add_pq since it is full" << "\tpacket->cpu = " << packet->cpu  << endl; });
        return -2; // cannot handle this request
    }
    ...
    cout << " event: " << PQ.entry[index].event_cycle << " current: " << current_core_cycle[PQ.entry[index].cpu] << "\tPQ.entry[index].cpu = " << PQ.entry[index].cpu  << endl; });
...
void CACHE::llc_prefetcher_operate(uint32_t cpu, uint64_t addr, uint64_t ip, uint8_t cache_hit, uint8_t type) {
    ...
    llc_prefetcher_operate_(cpu, addr, ip, cache_hit, type); /* delegate */
}
